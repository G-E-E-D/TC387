...
#define DISPLAY_PAGE_COUNT       (4U)
#define DISPLAY_ROWS             (10U)
#define DISPLAY_PAGE_REFRESHES   (20U)
...
void vehicle_display_next_page(VehicleDisplay *display)
{
    if(display != NULL)
    {
        display->page = (uint8_t)((display->page + 1U) % DISPLAY_PAGE_COUNT);
        display->refreshes_on_page = 0U;
    }
}
...
{
    if((display == NULL) || (t == NULL) || (writer == NULL))
    {
        return;
    }
    write_line(writer, context, 0U, "%s P%u CAL:%c", vehicle_state_name(t->state),
               (unsigned int)display->page, calibration_valid ? 'Y' : 'N');
    switch(display->page)
    {
        case 0U:
            write_line(writer, context, 1U, "ENC raw L:%ld R:%ld",
                       (long)t->left_wheel.delta_count, (long)t->right_wheel.delta_count);
            write_line(writer, context, 2U, "ENC ext L:%lld", (long long)t->left_wheel.count);
            write_line(writer, context, 3U, "ENC ext R:%lld", (long long)t->right_wheel.count);
            write_line(writer, context, 4U, "SPD L:%+.3f R:%+.3f",
                       (double)t->left_wheel.speed_mps, (double)t->right_wheel.speed_mps);
            write_line(writer, context, 5U, "MT raw:%u stat:%02X", (unsigned int)t->steering.raw_angle,
                       (unsigned int)t->steering.magnetic_status);
            write_line(writer, context, 6U, "MT ext:%lld", (long long)t->steering.continuous_count);
            write_line(writer, context, 7U, "MT rel:%lld", (long long)t->steering.relative_count);
            write_line(writer, context, 8U, "STEER:%+.4f rad", (double)t->steering.angle_rad);
            write_line(writer, context, 9U, "FAULT:%08lX", (unsigned long)t->fault_flags);
            break;
        case 1U:
            ...
            write_line(writer, context, 5U, "GYRO bias:%+.6f", (double)t->pose.gyro_z_bias_radps);
            write_line(writer, context, 6U, "OMEGA I:%+.3f W:%+.3f",
                       (double)t->pose.omega_imu_radps, (double)t->pose.omega_wheel_radps);
            write_line(writer, context, 7U, "OMEGA S:%+.3f", (double)t->pose.omega_steering_radps);
            write_line(writer, context, 8U, "IMU:%c MAG:%c", t->imu.accel_gyro_valid ? 'Y' : 'N',
                       t->imu.magnetometer_valid ? 'Y' : 'N');
            write_line(writer, context, 9U, "FAULT:%08lX", (unsigned long)t->fault_flags);
            break;
        case 2U:
            write_line(writer, context, 1U, "X:%+.3f Y:%+.3f", (double)t->pose.x_m,
                       (double)t->pose.y_m);
            write_line(writer, context, 2U, "YAW:%+.4f", (double)t->pose.yaw_rad);
            write_line(writer, context, 3U, "V:%+.3f STATIC:%c", (double)t->pose.vehicle_speed_mps,
                       t->pose.stationary ? 'Y' : 'N');
            write_line(writer, context, 4U, "PATH n:%lu s:%.2f",
                       (unsigned long)t->path_point_count, (double)t->path_length_m);
            write_line(writer, context, 5U, "IDX n:%lu t:%lu",
                       (unsigned long)t->tracker.nearest_index,
                       (unsigned long)t->tracker.target_index);
            ...
            write_line(writer, context, 7U, "LOOK:%.3f SLIP:%c",
                       (double)t->tracker.lookahead_m, t->pose.wheel_slip ? 'Y' : 'N');
            write_line(writer, context, 8U, "TARGET V:%+.3f", (double)t->target_speed_mps);
            write_line(writer, context, 9U, "TARGET ST:%+.4f", (double)t->target_steering_rad);
            break;
        default:
            write_line(writer, context, 1U, "PWM L:%+.3f R:%+.3f",
                       (double)t->actuators.left_motor_duty,
                       (double)t->actuators.right_motor_duty);
            write_line(writer, context, 2U, "PWM ST:%+.3f", (double)t->actuators.steering_motor_duty);
            write_line(writer, context, 3U, "FAULT:%08lX", (unsigned long)t->fault_flags);
            write_line(writer, context, 4U, "LATCH:%08lX", (unsigned long)t->latched_fault_flags);
            write_line(writer, context, 5U, "LOG DROP:%lu", (unsigned long)log_dropped_lines);
            write_line(writer, context, 6U, "TS:%llu us", (unsigned long long)t->timestamp_us);
